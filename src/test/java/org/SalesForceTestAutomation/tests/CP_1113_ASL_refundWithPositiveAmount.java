...
public class CP_1113_ASL_refundWithPositiveAmount extends TestBase {
	...
	String returnAmount = readPro.getReturnAmountPayment();
...
//		SalesForce_Opportunity opportunity = new SalesForce_Opportunity();
		...
		// Start Test
		System.out.println("Step 1:Start Test: CP_1113_ASL_refundWithPositiveAmount");
		logger.info("Step 1:Start Test: CP_1113_ASL_refundWithPositiveAmount");
		ExtentReport.initialisation("CP_1113_ASL_refundWithPositiveAmount");
		...
		String msg = opportunity.cp_1113_ASL_refundwithPositiveAmount(opportunityName, donationAmount, returnAmount);
		System.out.println("Step 5:Set the payment as paid and try to refunt it using positive value.");
		ExtentReport.logStat(Status.PASS, "Step 5:Set the payment as paid and try to refunt it using positive value.");
		logger.info("Step 5:Set the payment as paid and try to refunt it using positive value.");

		Assert.assertEquals(msg, "An Original Payment is required for to a refund payments. Otherwise leave blank.");
		System.out.println("Step 6:Validate the validation message is"
				+ "An Original Payment is required for to a refund payments. Otherwise leave blank.");
		ExtentReport.logStat(Status.PASS, "Step 6:Validate the validation message is"
				+ "An Original Payment is required for to a refund payments. Otherwise leave blank.");
		logger.info("Step 6:Validate the validation message is"
				+ "An Original Payment is required for to a refund payments. Otherwise leave blank.");

		System.out.println("Step 7:End Test: CP_1113_ASL_refundWithPositiveAmount");
		ExtentReport.logStat(Status.PASS, "Step 7:End Test: CP_1113_ASL_refundWithPositiveAmount");
		logger.info("Step 7:End Test: CP_1113_ASL_refundWithPositiveAmount");
	
		}
		catch (Exception e){
			ExtentReport.logStat(Status.FAIL,
					"'CP_1113_ASL_refundWithPositiveAmount' test got failed and the error is - "
							+ e);
